...
export function OAuthButtons() {
  ...
  async function signIn(provider: Provider) {
    ...
    if (!isSupabaseConfigured()) {
      setError(
        "Authentication is not configured correctly. Add NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY to .env.local.",
      );
      return;
    }
    ...
    try {
      ...
      const { error: oauthError } = await supabase.auth.signInWithOAuth({
        ...
        options: {
          ...
          ...(provider === "google"
            ...
            : {
                // Read profile + email for dashboard display.
      ...
      if (oauthError) {
        ...
        if (message.includes("provider is not enabled")) {
          setError(
            `${provider === "github" ? "GitHub" : "Google"} sign-in is not enabled yet. Enable it in Supabase → Authentication → Providers.`,
          );
        } else {
